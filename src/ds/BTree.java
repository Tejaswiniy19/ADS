 ...
 * (Modified from B+Tree to serve as a structurally similar B-Tree variant)
 */
public class BTree {
    private int m;
    ...
    private class InternalNode extends Node {
        List<Node> children;
        InternalNode() { super(); children = new ArrayList<>(); }
        @Override boolean isOverflow() { return children.size() > m; }
        @Override SplitResult split() {
            int mid = keys.size() / 2;
    ...
    private class LeafNode extends Node {
        ...
        LeafNode() { super(); values = new ArrayList<>(); }
        @Override boolean isOverflow() { return values.size() >= m; }
        @Override SplitResult split() {
            int mid = keys.size() / 2;
            long upKey = keys.get(mid);
